...
pub fn authenticate(
    ...
    // Clients sign the complete public path, including a reverse-proxy mount.
    // Use configured endpoint only; never trust a caller-supplied forwarded path.
    let endpoint = reqwest::Url::parse(&config.public_url)
        .map_err(|e| Error::Internal(format!("invalid public endpoint: {e}")))?;
    let public_path = format!("{}{}", endpoint.path().trim_end_matches('/'), path);
...
mod tests {
    ...
    #[test]
    fn proxy_prefix_is_part_of_the_signed_path() {
        let (signing, mut config, store, now, stamp) = fixtures();
        config.public_url = "https://example.com/simple-backup".into();
        let good = signed_headers(&signing, "GET", "/simple-backup/v1/snapshots", b"", &stamp, "good");
        assert!(authenticate(&good, "GET", "/v1/snapshots", b"", &config, &store, now).is_ok());
        for path in ["/v1/snapshots", "/other/v1/snapshots"] {
            let bad = signed_headers(&signing, "GET", path, b"", &stamp, "bad");
            assert!(authenticate(&bad, "GET", "/v1/snapshots", b"", &config, &store, now).is_err());
        }
    }
