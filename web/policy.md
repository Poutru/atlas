...
Anyone may submit an HTTPS manifest URL without an account. We verify a bounded UTF-8 JSON document, reject duplicate keys and noncanonical number forms, verify its Ed25519 signature under the declared operator key, validate version 1, componentId, seat and published expiry where applicable. The storage.backup role and the explicit onym:naming-implementation:bsn-stellar-http-v1 registry profile may omit validUntil under their manifest shapes. BSN manifests must use seat naming.association, the qualified-association-v1 naming profile, a matching Ed25519 operator/trust-root key, registry, namespace and policy digest; their signature is verified over the exact onym-bsn-np-v1:manifest\n domain prefix followed by canonical unsigned JSON. Other manifests retain their existing signature and expiry rules. Missing manifest expiry does not remove the hourly availability checks or the signed snapshot expiry; naming records and resolution responses have their own lifetimes verified by clients. All syntactically valid seat types are eligible. These are baseline manifest checks, not a complete implementation of every destination seat's conformance suite. The destination client must verify seat-specific schemas, profiles, compatibility, consent and operation before use. Unsupported claims never grant capabilities.
